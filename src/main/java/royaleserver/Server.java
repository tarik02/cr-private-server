...
public class Server {
	...
	public void removePlayer(Player player) {
		players.remove(player.entity.getId(), player);
	}

	public Clan resolveClan(Player player) {
		ClanEntity clanEntity = player.entity.getClan();
		if (clanEntity == null) {
			return null;
		}

		if (player.clan != null) {
			if (player.clan.getId() == clanEntity.getId()) {
				return player.clan;
			}

			player.clan.removePlayer(player);
		}

		Clan clan = clans.computeIfAbsent(clanEntity.getId(), Clan::new);
